...
int get_nth_number(int index, char *value, char delimit, int base)
{
	char str[64] = {0};
	//int num = 0;
...
int get_ssid_index(char* ifname, char* prefix)
{
	char *start;
	...
	if(strlen(prefix) >= strlen(ifname))
		return -3;

	start = strstr(ifname, prefix);
	if(start != NULL)
		return (int)strtol((ifname + strlen(prefix)), NULL, 10);
	else
		return -4;
}
...
FILE *fetch_media_cfg(struct media_config *cfg, int write)
{
	...
	while(EOF != fscanf(mfp, "%*s %50s %*s %*s %*s %*s\n", path))
	{
		if (!strncmp(path, "/media/sd", 9) || 
		    !strncmp(path, "/media/mmc", 10)) {
			snprintf(path, sizeof(path),"%s/.media_config", path);
...
int is_ipnetmask_valid(char *s)
{
	...
	if (!is_allnum_slash(&str[0])){
		return 0;
	}

	if(!is_one_slash(&str[0])){
		return 0;
	}
...
void do_system_redirect(char *fmt, ...)
{
	...
	vsprintf(cmd, fmt, ap);
	va_end(ap);
	snprintf(cmd, sizeof(cmd),"%s 1>%s 2>&1", cmd, "tmp");
	system(cmd);
...
void update_flash_8021x(int nvram)
{
	...
	// OperationMode is only saved in RT2860_NVRAM.
...
void lookupAllList(char *dir_path)
{
	...
	while((dir = readdir(dirp)))
	{
			...
			(4 == strlen(dir->d_name)-(int)loc+(int)dir->d_name))
		{
			...
			strncpy(all_list[i].name, dir->d_name, sizeof(all_list[i].name));
			//strcpy(all_list[i].path, whole_path);
			strncpy(all_list[i].path, whole_path, sizeof(all_list[i].path));
			i++;
		}
	}
...
void lookupSelectList(void)
{
	...
	snprintf(track_path, sizeof(track_path),"%s/track.txt", track_path);
